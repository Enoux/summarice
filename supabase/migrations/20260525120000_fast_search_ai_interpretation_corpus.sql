-- Fast library search: full highlight search corpus including AI interpretation for partial match.
...
as $$
  ...
  select
    ...
    trim(
      coalesce(h.text, '') || ' ' ||
      coalesce(h.comment, '') || ' ' ||
      coalesce(ar.annotation_preview, '') || ' ' ||
      coalesce(ar.ai_annotation_preview, '')
    ),
    to_tsvector(
      'english',
      trim(
      ...
      greatest(
        ...
        case
          when public.corpus_matches_partial(
            lower(coalesce(nullif(p.search_corpus, ''), trim(
              coalesce(p.text, '') || ' ' ||
              coalesce(p.comment, '') || ' ' ||
              coalesce(p.annotation_preview, '') || ' ' ||
              coalesce(p.ai_annotation_preview, '')
            ))),
            p_partial_terms,
            p_partial_excluded
