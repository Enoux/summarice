import groupHighlightsByPage from './pdf_utils/group-highlights-by-page';
import type { Highlight, ViewportHighlight } from './types';
...
export class HighlightsModel<T extends Highlight> {
	...
	private set highlights(new_highlights: Array<T>) {
		this._highlights = new_highlights;
		this.listeners.forEach((listener) => {
			const response = listener(this._highlights);
	...
	public subscribe = (callback: (arr: Array<T>) => unknown) => {
	...
	public addGhostHighlight = (hl: T) => {
		this.highlights = [...this.highlights, { ...hl, id: 'TEMP' + this.getNextId(), is_temp: true }];
	};
	public removeGhostHighlights = () => {
		this.highlights = this.highlights.filter((item) => !item.is_temp);
	};
