...
<script lang="ts">
	...
	type HighlightUtils = {
		...
		viewportToScaled: (rect: LTWHP) => ReturnType<typeof viewportToScaled>;
		screenshot: (boundingRect: LTWH) => ReturnType<typeof screenshot>;
		highlightBindings: HighlightBindings;
	};
	...
	const getScaledHighlights = (highlights: Highlight[] | undefined): HighlightUtils[] => {
		...
		return new_currentHighlights.filter((item): item is HighlightUtils => item !== null);
	};
	...
	$effect.pre(() => {
		...
		void pdfHighlighterUtils.currentScale; //dependence
		a_currentHighlights = getScaledHighlights(currentHighlights);
		//console.log('run HL effect, page:' + pageNumber);
	});

	const getCurrentHighlightsInOrder = (highlights: HighlightUtils[]): HighlightUtils[] => {
		...
		return [...highlights].sort((a, b) => (a.highlight.z_index ?? 0) - (b.highlight.z_index ?? 0));
